...
public class TP02Q05 {
    public static void main(String[] args) {
        ...
        ArrayList<Jogador> playerselected = new ArrayList<>();
        String id = scanner.nextLine();
        while (!id.equals("FIM")) {
            playerselected.add(jogadores.get(Integer.parseInt(id)));
            id = scanner.nextLine();
        }

        int[] contador = selectionSort(playerselected);

        for (int i = 0; i < playerselected.size(); i++) {
            playerselected.get(i).imprimirDados();
        }
        ...
        try{
            ...
            arqWriter.write("Matricula: 804103\tTempo: " +(fim - inicio) / 1000d + "\tComparacoes: "+ contador[0] + "\tMovimentações: " + contador[1]);
            arqWriter.close();
        }catch(Exception e){
            System.out.println(e.getMessage() + "\n" + e.getLocalizedMessage());
        }
    ...
    public static int[] selectionSort(ArrayList<Jogador> playerselected) {
        int n = playerselected.size();
        int comparacoes = 0;
        int movimentacoes = 0;
        for (int i = 0; i < n-1; i++) {
           int menor = i;
           for (int j = (i + 1); j < n; j++){
            comparacoes++;
            if (playerselected.get(j).getNome().compareToIgnoreCase(playerselected.get(menor).getNome()) < 0) {
                menor = j;
              }
           }
           swap(playerselected,menor, i);
              movimentacoes +=3;
        }
        return new int[] {comparacoes,movimentacoes};
     }
     public static void swap(ArrayList<Jogador> playerselected, int i, int j) {
        Jogador temp = playerselected.get(i);
        playerselected.set(i, playerselected.get(j));
        playerselected.set(j, temp);
    }
}