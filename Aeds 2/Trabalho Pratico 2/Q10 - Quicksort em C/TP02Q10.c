...
typedef struct Jogador{
    ...
    void quicksort(Jogador jogadorID[], int esq, int dir){
        int i = esq, j = dir;
        Jogador pivo = jogadorID[(esq + dir) / 2];
        while(i <= j){
            //caso o estado de nascimento de um esteja antes na ordem alfabética ou caso seja igual o nome de um esteja na frente do outro
            while(strcmp(getEstadoNascimento(&jogadorID[i]), getEstadoNascimento(&pivo)) < 0 ||(strcmp(getEstadoNascimento(&jogadorID[i]), getEstadoNascimento(&pivo)) == 0 && strcmp(getNome(&jogadorID[i]), getNome(&pivo)) < 0)) i++;

            while(strcmp(getEstadoNascimento(&jogadorID[j]), getEstadoNascimento(&pivo)) > 0 ||(strcmp(getEstadoNascimento(&jogadorID[j]), getEstadoNascimento(&pivo)) == 0 && strcmp(getNome(&jogadorID[j]), getNome(&pivo)) > 0))j--;


            if(i <= j){
                swap(&jogadorID[i], &jogadorID[j]);
                i++;
                j--;
            }
        }
        if (esq < j)  quicksort(jogadorID, esq, j);
        if (i < dir)  quicksort(jogadorID, i, dir);
    }
...
int main(void) {
    ...
    quicksort(jogadorID, 0, controle);
