...
public class Experiment {
    ...
    // Tamanho maximo de vertices para Fleury com Tarjan (O(E*(V+E)) impratica para V grande)
    private static final int TARJAN_FLEURY_MAX = 10_000;

    // Tamanho do grafo usado para exibir resultados funcionais (pontes e caminho)
    private static final int FUNCTIONAL_REPORT_SIZE = 100;

    // Limite de tempo por execução (ms). Ao exceder, mostramos N/A (lento).
    ...
    public static void runExperiments() {
        ...
        BridgeFinder[] finders = {new NaiveBridgeFinder(), new TarjanBridgeFinder()};
        ...
        for (int V : sizes) {
            for (int typeId = 0; typeId < 3; typeId++) {
                ...
                for (BridgeFinder finder : finders) {
                    ...
                    long avgMs = measureBridgeFinding(g, finder);
                    printRow(typeName(typeId), V, E, finder.getName(),
                             avgMs >= 0 ? avgMs + " ms" : "N/A (lento)");
                }
            }
        }
        ...
        for (int V : sizes) {
            for (int typeId = 0; typeId < 2; typeId++) { // apenas Euleriano e Semi
                Graph g = buildGraph(gen, V, typeId);
                int E = g.E;
                ...
                for (BridgeFinder finder : finders) {
                    ...
                    long avgMs = measureFleury(g, finder);
                    printRow(typeName(typeId), V, E, finder.getName(),
                             avgMs >= 0 ? avgMs + " ms" : "N/A (lento)");
                }
            }
            // Não-Euleriano: Fleury retorna null imediatamente
    ...
    // -------------------------------------------------------------------------
    // Saída funcional: imprime pontes e caminho euleriano para cada tipo de grafo
    // -------------------------------------------------------------------------
    private static void runFunctionalReport(GraphGenerator gen, BridgeFinder[] finders) {
        ...
        for (int typeId = 0; typeId < 3; typeId++) {
            ...
            for (BridgeFinder finder : finders) {
                ...
                if (path != null) {
                    System.out.printf("Arestas percorridas (%s): %d / %d%n",
                                      finder.getName(), path.size() - 1, g.E);
                }
            }
        }
    }
    ...
    // -------------------------------------------------------------------------
    // Medição de tempo: findAllBridges
    // -------------------------------------------------------------------------
    private static long measureBridgeFinding(Graph original, BridgeFinder finder) {
        long total = 0;
    ...
    // -------------------------------------------------------------------------
    // Medição de tempo: Fleury completo
    // -------------------------------------------------------------------------
    private static long measureFleury(Graph original, BridgeFinder finder) {
        FleuryAlgorithm fleury = new FleuryAlgorithm(finder);
        long total = 0;
        for (int r = 0; r < NUM_RUNS; r++) {
            long t0 = System.currentTimeMillis();
    ...
    // -------------------------------------------------------------------------
    // Construção dos grafos de teste
    // -------------------------------------------------------------------------
    private static Graph buildGraph(GraphGenerator gen, int V, int typeId) {
        switch (typeId) {
            case 0: return gen.generateEulerian(V);
            case 1: return gen.generateSemiEulerian(V);
            default: return gen.generateNonEulerian(V);
        }
    }

    private static String typeName(int id) {
        switch (id) {
            case 0: return "Euleriano";
            case 1: return "Semi-Euler.";
            default: return "Não-Euler.";
        }
    }
    ...
    private static String pathToString(List<Integer> path) {
        if (path == null) return "não existe (grafo não euleriano)";
    ...
    private static void printRow(String tipo, int V, int E,
                                  String method, String time) {
        System.out.printf("%-14s %-10d %-10d %-10s %-20s%n",
                          tipo, V, E, method, time);
    }
}